...
struct alignas(16) GPUBVHNode   // 强制整个结构体的对齐方式为 16 字节边界
{
    ...
    float primType;         //  0=球体, 1=立方体  
    float pad1, pad2;       // 凑齐16字节，避免下一个node因为对齐产生偏移
};
...
static_assert(sizeof(GPUBox) == 32, "GPUBox size mismatch, check alignment");
...
class BVHFlattener
{
    public:
        ...
        std::vector<int> flatLightIndices;  // 记录哪些下标的球是发光体
        std::vector<GPUBox> flatBoxes; 
    ...
    private:
        ...
        int flattenNode(shared_ptr<hittable> node)
        {
            ...
            {
                auto asSphere = std::dynamic_pointer_cast<sphere>(node);
                auto asBox    = std::dynamic_pointer_cast<box>(node);

                aabb bbox;
                node->bounding_box(bbox);

                GPUBVHNode gpuNode{};
                gpuNode.aabbMin = glm::vec3(bbox.min().x(), bbox.min().y(), bbox.min().z());
                gpuNode.aabbMax = glm::vec3(bbox.max().x(), bbox.max().y(), bbox.max().z());
                gpuNode.leftChild = -1;
                gpuNode.isLeaf = 1;
                ...
                {
                    GPUSphere gpuSphere{};
                    gpuSphere.center = glm::vec3(asSphere->center.x(), asSphere->center.y(), asSphere->center.z());
                    gpuSphere.radius = (float)asSphere->radius;
                    gpuSphere.materialId = flattenMaterial(asSphere->mat);

                    flatSpheres.push_back(gpuSphere);
                    int sphereIdx = (int)flatSpheres.size() - 1;

                    gpuNode.rightChild = sphereIdx;
                    gpuNode.primType = 0.0f;

                    if (flatMaterials[gpuSphere.materialId].type == 3) 
                    {
                        flatLightIndices.push_back(sphereIdx); 
                    }
                } 
                else if (asBox) 
                {
                    GPUBox gpuBox{};
                    gpuBox.boxMin = glm::vec3(asBox->get_min().x(), asBox->get_min().y(), asBox->get_min().z());
                    gpuBox.boxMax = glm::vec3(asBox->get_max().x(), asBox->get_max().y(), asBox->get_max().z());
                    gpuBox.materialId = flattenMaterial(asBox->get_material());

                    flatBoxes.push_back(gpuBox);
                    int boxIdx = (int)flatBoxes.size() - 1;

                    gpuNode.rightChild = boxIdx;
                    gpuNode.primType = 1.0f;
                }
